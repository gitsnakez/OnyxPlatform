...
{
public:
	static std::string ConvertWstrToStr(std::wstring input)
	{
	...
	static const char* ConvertWcharToChar(const wchar_t* input)
	{
		// Вычисляем необходимый размер буфера для строки типа string
	...
	static const wchar_t* ConvertCharToWchar(const char* input)
	{
	...
	static std::wstring ConvertStrToWstr(std::string input)
	{
		// Get the length of the UTF-16 string

...
namespace OSharp
{
    ...
    {
        public static void FlashApplicationWindow(Window window)
        {
            IntPtr winHandle = StartBlink(window);
            Flash(winHandle, 5);
        }
        ...
        public static void StopFlashing(Window window)
        {
            ...
            if (Win2000OrLater)
            {
                FLASHWINFO fi = CreateFlashInfoStruct(winHandle, FLASHW_STOP, uint.MaxValue, 0);
                FlashWindowEx(ref fi);
            }
        }

        private static IntPtr StartBlink(Window window)
        {
